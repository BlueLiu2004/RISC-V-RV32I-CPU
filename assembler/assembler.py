...
def format_output(items: list[tuple[int, AsmLine]], fmt: str) -> str:
	rows: list[str] = []
	for idx, (code, ins) in enumerate(items):
		if fmt == "hex":
			rows.append(f"{code:08x}")
		elif fmt == "bin":
			rows.append(f"{code:032b}")
		elif fmt == "imem":
			rows.append(f"imem[{idx}] = 32'h{code:08x}; // L{ins.line_no}: {ins.text}")
		else:
			raise AssemblerError(f"未知輸出格式: {fmt}")
	return "\n".join(rows) + ("\n" if rows else "")
...
def main() -> int:
	...
	parser.add_argument("--format", choices=["hex", "imem", "bin"], default="imem", help="輸出格式")
	...
	try:
		...
		rendered = format_output(items, args.format)
